...
export default function Track({ questions, addQuestion, updateQuestion, deleteQuestion, toggleBookmark, markAsSolved }) {
    ...
    return (
        <div className="relative min-h-[80vh]">
            <div className="flex justify-between items-center mb-8 sticky top-0 bg-slate-50/90 backdrop-blur z-10 py-4 border-b border-slate-200">
                ...
                {Object.entries(groupedQuestions).map(([group, items]) => (
                    <div key={group} className="glass-card break-inside-avoid mb-6">
                        ...
                        <div className="grid gap-4">
                            {items.map(q => (
                                <div key={q.id} className="bg-white p-4 rounded-lg border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="flex justify-between items-start mb-2">
                                        <h4 className="font-bold text-slate-800">{q.title}</h4>
                                        <button onClick={() => toggleBookmark(q.id)} className={`text-lg ${q.isBookmarked ? 'text-yellow-500' : 'text-slate-300'}`}>
                                            ★
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center mt-2 pt-2 border-t border-slate-50">
                                        <div className="flex gap-2">
                                            {q.status !== 'Solved' && (
                                                <button onClick={() => markAsSolved(q.id)} className="text-xs text-green-600 font-medium hover:underline">
                                                    Mark Solved
                                                </button>
                                            )}
                                            {q.link && (
                                                <a href={q.link} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline">
                                                    Link ↗
                                                </a>
                                            )}
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => handleEdit(q)} className="text-xs text-slate-400 hover:text-slate-600">
                                                Edit
                                            </button>
                                            <button onClick={() => deleteQuestion(q.id)} className="text-xs text-red-400 hover:text-red-600">
                                                Delete
                                            </button>
                                        </div>
                                    </div>
